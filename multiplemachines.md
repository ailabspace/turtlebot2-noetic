...
## Automate robot bringup (incomplete)
...
- Auto bringup Turtlebot
  ...
  - It is convenient for the Turtlebot bringup to run upon login.
    ...
    - Unfortunately, `roslaunch` does not work from "Startup Applications". We will use the `robot_upstart` package to start Turtlebot bringup launch file when the system boot.
...
## Shutdown by power switch

- If the robot is running **headless**, the easiest way to shut it down is by pressing the power button. Search for "Power" in your applications. Set the power button to "Power Off".
  
![Power off](img/power_off.png)

- Pressing the power button will bring up the power off pop up. The machine will power off in 60 seconds. On some machines, you may need to press and hold the power button for a few seconds (but not too long). Test your machine with monitor to find out.
